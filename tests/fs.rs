...
fn partial_fs() {
    ...
    let private_path = fs::canonicalize(NamedTempFile::new().unwrap()).unwrap();
    fs::write(&private_path, FILE_CONTENT.as_bytes()).unwrap();
    let public_path = fs::canonicalize(NamedTempFile::new().unwrap()).unwrap();
    fs::write(&public_path, FILE_CONTENT.as_bytes()).unwrap();

    // Activate our sandbox.
    let mut birdcage = Birdcage::new().unwrap();
    birdcage.add_exception(Exception::Read(public_path.clone())).unwrap();
